  ...
  <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-pulse">
    <div className="flex flex-col lg:flex-row gap-8">
      ...
      <div className="lg:w-3/4">
        {/* Ad Banner Skeleton */}
        <div className="min-h-[90px] w-full bg-gray-200 dark:bg-gray-700 rounded-lg mb-4"></div>
        <div className="aspect-video mb-4 bg-gray-200 dark:bg-gray-700 rounded-lg"></div>
      ...
      <div className="lg:w-1/4">
        <div className="h-8 w-32 mb-4 bg-gray-200 dark:bg-gray-700 rounded"></div>
        <div className="space-y-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="flex items-start gap-4 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
              <div className="w-2/5 h-20 bg-gray-200 dark:bg-gray-700 rounded-md"></div>
...
const WatchPage: React.FC = () => {
  ...
  const upNextItems = useMemo(() => {
    const items: JSX.Element[] = relatedVideos.map(relatedVideo => (
      <Link to={`/video/${relatedVideo.slug}`} key={relatedVideo.slug} className="flex items-start gap-4 bg-gray-50 dark:bg-gray-800 p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 transition-colors duration-200 group">
        <div className="w-2/5 flex-shrink-0">
          <img src={relatedVideo.thumbnail} alt={relatedVideo.title} className="rounded-md aspect-video object-cover" />
        </div>
        <div className="w-3/5">
          <h3 className="text-md font-semibold text-gray-900 dark:text-white group-hover:text-red-600 transition-colors duration-200 line-clamp-2">{relatedVideo.title}</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-1">{relatedVideo.category}</p>
        </div>
      </Link>
    ));
  ...
  if (!video) {
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-gray-900 dark:text-white text-2xl">Video not found.</div>
      </div>
    );
  }
    ...
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-3/4">
          ...
          <h1 className="text-3xl lg:text-4xl font-extrabold text-gray-900 dark:text-white mb-3">{video.title}</h1>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-gray-600 dark:text-gray-300 mb-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed">{video.description}</p>
          </div>
          
          {/* Premium Access CTA */}
          <div className="bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 p-6 rounded-lg border border-red-200 dark:border-red-800 mt-6">
            <div className="text-center">
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Want More Premium Content?</h3>
              <p className="text-gray-700 dark:text-gray-300 mb-4">Unlock exclusive videos and premium features</p>
        ...
        {/* Sidebar */}
        <div className="lg:w-1/4">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 border-l-4 border-red-500 pl-4">Up Next</h2>
          <div className="space-y-4">
